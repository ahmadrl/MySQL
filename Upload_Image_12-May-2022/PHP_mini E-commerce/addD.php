...
<nav class="navbar navbar-expand-md bg-dark navbar-dark black">
   ...
   if(isset($_POST['sub']))
    ...
    if (!empty($_POST))
     { 
      $p_image = $_FILES['prImg']['name'];
      $p_image_tmp_name = $_FILES['prImg']['tmp_name'];
      $p_image_folder = 'images/'.$p_image;
      $arrr=array('pname'=>$_POST['pr'],'quantity'=>$_POST['prQuantity'],'desc'=>$_POST['prDesc']);
      $_SESSION['arr1'][]= $arrr;

      $insert_query = mysqli_query($conn, "INSERT INTO `products`(image) VALUES('$p_image')")or die('query failed') ;
      if($insert_query){
